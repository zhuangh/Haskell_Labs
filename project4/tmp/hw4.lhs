...
> evalE (Var x)      = do
>                    memory <- get
>                    case (Data.Map.lookup x memory) of
>                      Nothing  -> do
>                                  evalS $ Print ("Error: Reading an undefined Var " ++ x ++ "! ") (Val (IntVal 0))
>                                  throwError (IntVal 0)
>                      Just val -> return val
...
>                      (IntVal v1, _) -> do
>                                   evalS $ Print ("Error: Binary operator " ++ (show o) ++ " is not applied to IntVal! ") e2
>                                   throwError (IntVal 2)
>                      otherwise -> do
>                                   evalS $ Print ("Error: Binary operator " ++ (show o) ++ " is not applied to IntVal! ") e1
>                                   throwError (IntVal 2)
>              where divide n m = if m == 0 then do
>                                                evalS $ Print "Error: Division by zero! " e2
>                                                throwError (IntVal 1)
>                                           else return (IntVal (n `div` m))
...
>                          otherwise -> do
>                                    evalS $ Print "Error: While-Condition is not a BoolVal! " e
>                                    throwError (IntVal 2)
> evalS Skip             = return ()
...
>                          otherwise -> do
>                                    evalS $ Print "Error: If-Condition is not a BoolVal! " e
>                                    throwError (IntVal 2)
> evalS (Print s e)      = do
>                        val <- evalE e
>                        tell (s ++ (show val) ++ "\n")
...
>                        where handler err = do
>                                      memory <- get
>                                      put (insert x err memory)
>                                      evalS h
...
>         where ((err, log), st') = runState (runWriterT (runErrorT (evalS s))) st
>               exn = case err of
>                       Left a  -> Just a
>                       Right _ -> Nothing
...
>   case (binary xs) of
>     0 -> (binary (sampleN out), binary [sample1 cout]) == (0, binary (sample1 cin))
...
> halfsub (x, y) = (diff, cout) -- (x - y)
>   where diff  = xor2 (x, y)
>         notx  = xor2 (x, high)
>         cout  = and2 (notx, y)

> bitSub :: (Signal, [Signal]) -> ([Signal], Signal)
> bitSub (ci, [])   = ([], ci)
> bitSub (ci, x:xs) = (df:dfs, co)
>   where (df,  c)  = halfsub (x, ci)
>         (dfs, co) = bitSub (c, xs)

> bitSubtractor :: (Signal, [Signal]) -> ([Signal], Signal)
> bitSubtractor (bit, xs) = (diff, cout)
>   where (dfs, cout) = bitSub (bit, xs)
>         (dump, diff) = demuxN (dfs, cout)
...
but youll need a little more wiring. To get an idea of how the
...
>                 case ys of
>                   [] -> xsy
>                   otherwise -> adder (xsy, [low] ++ multiplier (xs, ys))
>   where (xsy, dump) = demuxN (xs, y)

> test6 = probe [ ("x1", x1), ("x2",x2),
>                 (" y1",y1),
>                 (" s1",s1), ("s2",s2) ]
...
>         [s1,s2,s3,s4,s5,s6,s7] = multiplier (xs, ys)
