...
The `foldl` function applies a binary function to an accumulator data type and
an element of a list from left to right. At the beginning of folding, the
binary function takes an initial accumulator value as the first argument and
the head element as the second, and produces the next accumulator value. Then
`foldl` passes the binary function, the next accumulator value and the next
list element recursively to itself until the last element of the list. The
final result of `foldl` is the a value of the accumulator data type.
...
> -- planets = orbit (orbit moon earth 2.0 0.8 0.4) (orbit mercury sun 2.0 2.0 0.2) 1.0 3.0 0.9
> -- planets = orbit ( orbit (orbit moon earth 2.0 0.5 0.2)  mercury  2.0 2.0 0.2) sun 1.0 3.0 0.3 
...
>       -> Behavior Picture
> orbit satl body freq xrad yrad = orbitB satl body (pure freq) (pure xrad) (pure yrad)


> orbitB :: Behavior Picture -- the satellite
>        -> Behavior Picture -- the fixed body
>        -> Behavior Float   -- the frequency of the orbit
>        -> Behavior Float   -- the x-radius of the orbit
>        -> Behavior Float   -- the y-radius of the orbit
>        -> Behavior Picture
> orbitB satl body freqB xradB yradB = cond (0 >* sin (time * freqB)) 
