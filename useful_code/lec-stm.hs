...
{--
prefixes :: [a] -> [[a]] 
prefixes xs@(xs':_) = prefixes xs' :  xs
prefixes _ = []
--}


len ls@(l:_) = "List starts with " ++ show l ++ " and s " ++ show (length ls) ++ " items long"
len [] = "List is empty!"
...
lPhone (Phone (H hm) (W wk)) =  
    Phone (H (map toLower hm)) (W (map toLower wk))
...
{--
mainXXX = do (page, time) <- timeit $ asyncMapM timeDownload urls
      --     printf "TOTAL download time: %.2fs\n" time
	   putStrLn $ page
--}	   
